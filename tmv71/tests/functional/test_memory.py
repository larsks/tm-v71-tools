...
def test_vfo_settings(initial, band, freq_band, rx_freq):
    assert (
        initial.program_memory[0].vfo_settings[band].list[freq_band].rx_freq == rx_freq
    )


@pytest.mark.parametrize(
    "band,freq_band",
    [(0, memory.Memory.FreqBand.band_144), (1, memory.Memory.FreqBand.band_430)],
)
def test_band_config(initial, band, freq_band):
    assert initial.program_memory[0].bands[band].display_mode == initial.DisplayMode.vfo
    assert initial.program_memory[0].bands[band].freq_band == freq_band
    assert initial.program_memory[0].bands[band].tx_power == initial.TxPower.high
