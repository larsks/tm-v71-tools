...
def test_set_port_speed(radio, serial):
    ...
    assert serial.rx.getvalue().endswith(b"W\x00\x21\x01\x03E")
...
def test_memory_restore(radio, serial):
    ...
    assert serial.rx.getvalue().endswith(
        b"W~\x00\x00" + fd.getvalue()[-256:] + b"W\x00\x00\x02\x00KE"
    )
...
def test_get_call_channel(radio, serial):
    serial.stuff(b"CC 0,0146520000,0,0,0,0,0,0,08,08," b"000,00600000,0,0000000000,0\r")
