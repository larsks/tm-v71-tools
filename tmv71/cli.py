...
TMV71_CONFIG = os.path.expanduser(
    os.path.join(os.environ.get("XDG_CONFIG_HOME", "~/.config"), "tmv71.json")
)
...
def fmt_dict(d, format=FORMATS.SHELL, table_format="grid", key=None):
    ...
    if format == FORMATS.SHELL:
        return "\n".join(
            "{}={}".format(k, shlex.quote("{}".format(v))) for k, v in d.items()
        )
...
def formatted(f):
    ...
    @click.option(
        ...
        "-K",
        multiple=True,
        help="Limit output to the specified key (may be " "specified multiple times)",
    )
    @functools.wraps(f)
...
def apply_options_from_schema(model, **kwargs):
    ...
    for fname, fspec in model.declared_fields.items():
        ...
        if fname == "band":
            ...
            options.append(
                click.option(
                    "--{}".format(display_name), type=int, **kwargs.get(fname, {})
                )
            )
        elif isinstance(fspec, schema.RadioFloat):
            options.append(
                click.option(
                    "--{}".format(display_name), type=float, **kwargs.get(fname, {})
                )
            )
    ...
    def _(ctx, *args, **kwargs):
        if not ctx.settings.no_clear:
            ...
            for i in range(ctx.settings.clear_retries + 1):
                ...
                raise click.ClickException("failed to communicate with the radio")
...
@click.option(
    "-f",
    "--config-file",
    default=TMV71_CONFIG,
    show_default=True,
    help="Path to a JSON configuration file",
)
@click.option(
    "--no-config", is_flag=True, help="Do not attempt to read any configuration file"
)
@click.option("-p", "--port", help="Path to a serial device")
@click.option(
    "-s", "--speed", type=click.Choice(api.PORT_SPEED), help="Set the port speed"
)
@click.option(
    "-K",
    "--no-clear",
...
@click.option(
    ...
    "--clear-retries",
    type=int,
    help="Number of times to retry clear operation " "before failing",
)
@click.option(
    "-v",
    "--verbose",
...
@band.command("mode")
@click.option("--vfo", "mode", flag_value=schema.BAND_MODE.index("VFO"))
@click.option("--mem", "--memory", "mode", flag_value=schema.BAND_MODE.index("MEM"))
@click.option("--call", "mode", flag_value=schema.BAND_MODE.index("CALL"))
@click.option("--wx", "--weather", "mode", flag_value=schema.BAND_MODE.index("WX"))
@click.argument("band", type=click.Choice(BAND_NAMES))
@click.pass_obj
@clear_first
...
@click.option("--medium", "--med", "power", flag_value=schema.TX_POWER.index("MED"))
@click.option("--high", "power", flag_value=schema.TX_POWER.index("HIGH"))
@click.argument("band", type=click.Choice(BAND_NAMES))
@click.pass_obj
@clear_first
...
@click.option(
    ...
    "--channels",
    multiple=True,
    help="Specify a single chanel (-c 1) or " "a range of channels (-c 1:10)",
)
@click.option(
    "-s", "--skip-deleted", is_flag=True, help="Do not export deleted channels"
)
@click.pass_obj
@clear_first
def export_channels(ctx, output, channels, skip_deleted):
    ...
    with output:
        ctx.api.export_channels(output, selected=selected, skip_deleted=skip_deleted)
...
@click.option(
    ...
    "--sync",
    is_flag=True,
    help="Delete channels from the radio " "that do not exist in input",
)
@click.option(
    "-c",
    "--channels",
...
def import_channels(ctx, input, sync, channels, ignore_errors):
    ...
    with input:
        ctx.api.import_channels(input, selected=selected, ignore_errors=ignore_errors)
...
@vfo.command("band")
@click.argument("band", type=click.Choice(BAND_NAMES))
@click.argument("freq_band", type=click.Choice(api.FREQUENCY_BAND), required=False)
@click.pass_obj
@clear_first
...
def read_block(ctx, output, _hexdump, address, length):
    ...
    if ":" in address and length != 0:
        raise ValueError("you cannot specify a length " "when using an address range")
    elif ":" in address:
    ...
    with output, ctx.api.programming_mode():
        ...
        while addr < addr_end:
            ...
            LOG.info(
                "reading %d bytes of memory from 0x%04X",
